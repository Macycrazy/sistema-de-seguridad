 ...
 */
class DatosVehiculoTest extends TestCase
{
    public function test_la_placa_queda_en_mayusculas_y_sin_guiones_ni_espacios(): void
    {
        foreach (['AB123CD', 'ab-123-cd', 'AB 123 CD', ' ab123cd '] as $tecleada) {
            $this->assertSame(
                'AB123CD',
                DatosVehiculo::normalizarPlaca($tecleada),
    ...
    public function test_una_placa_vacia_es_nula_y_no_una_cadena_vacia(): void
    {
        ...
        $this->assertNull(DatosVehiculo::normalizarPlaca(''));
        $this->assertNull(DatosVehiculo::normalizarPlaca('   '));
        $this->assertNull(DatosVehiculo::normalizarPlaca('- -'));
        $this->assertNull(DatosVehiculo::normalizarPlaca(null));
    }

    public function test_los_demas_datos_se_recortan_y_no_se_quedan_con_espacios_de_sobra(): void
    {
        $vehiculo = DatosVehiculo::desde(DatosVehiculo::CARRO, '  Toyota ', 'Corolla   LE', ' Gris  ', 'AB123CD');
    ...
    {
        $this->assertTrue(DatosVehiculo::desde()->vacio());
        $this->assertTrue(DatosVehiculo::desde(marca: '', modelo: '  ', color: '', placa: null)->vacio());
        $this->assertFalse(DatosVehiculo::desde(placa: 'AB123CD')->vacio());
    }

    public function test_un_vehiculo_vacio_es_valido_porque_la_gente_entra_caminando(): void
    {
        DatosVehiculo::desde()->exigirValido();
    ...
    public function test_un_vehiculo_a_medias_sin_placa_no_es_valido(): void
    {
        ...
        DatosVehiculo::desde(marca: 'Toyota', color: 'Gris')->exigirValido();
    }
    ...
    public function test_con_la_placa_basta_aunque_no_se_sepa_la_marca(): void
    {
        ...
        DatosVehiculo::desde(placa: 'AB123CD')->exigirValido();
    ...
    {
        $this->assertSame(
            DatosVehiculo::LARGO_PLACA,
            mb_strlen(DatosVehiculo::normalizarPlaca(str_repeat('A', 40))),
        );
    }
    ...
    public function test_la_descripcion_se_lee_de_un_vistazo(): void
    {
        $this->assertSame(
            ...
            DatosVehiculo::desde(DatosVehiculo::CARRO, 'Toyota', 'Corolla', 'Gris', 'AB123CD')->descripcion(),
        );

        $this->assertSame(
            'Moto · Bera BR-150 · Negro · AC456DF',
            DatosVehiculo::desde(DatosVehiculo::MOTO, 'Bera', 'BR-150', 'Negro', 'AC456DF')->descripcion(),
        );

        // Y no deja separadores sueltos cuando falta algún dato.
        $this->assertSame('Carro · AB123CD', DatosVehiculo::desde(placa: 'AB123CD')->descripcion());
        $this->assertSame('', DatosVehiculo::desde()->descripcion());
    }
    ...
    public function test_sin_vehiculo_no_se_guarda_ningun_tipo(): void
    {
        ...
        $aPie = DatosVehiculo::desde(DatosVehiculo::CARRO);
    ...
    {
        $this->assertSame(DatosVehiculo::CARRO, DatosVehiculo::desde(placa: 'AB123CD')->tipo);
    }

    public function test_la_moto_se_distingue_del_carro(): void
    {
        $moto = DatosVehiculo::desde(DatosVehiculo::MOTO, placa: 'AC456DF');
        $carro = DatosVehiculo::desde(DatosVehiculo::CARRO, placa: 'AB123CD');
    ...
    public function test_un_tipo_que_no_existe_se_trata_como_carro(): void
    {
        ...
        $this->assertSame(DatosVehiculo::CARRO, DatosVehiculo::desde('camion', placa: 'AB123CD')->tipo);
        $this->assertSame(DatosVehiculo::MOTO, DatosVehiculo::desde('  MOTO  ', placa: 'AC456DF')->tipo);
    }
}
